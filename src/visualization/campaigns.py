...
_found: dict[tuple, Campaign | None] = {}


def picked(coverage: Sequence[SetCoverage], window: Window) -> Campaign | None:
    """Return the best window for one feature, searching for it only once.

    Every panel that marks the window asks for the same one, and confirming a
    feature draws them all, so the search is kept rather than repeated. What a
    feature is measured against decides it entirely: which sets are on show,
    and the date range they are limited to.
    ...
    if key not in _found:
        if len(_found) >= configs.CAMPAIGN_CACHE:
            _found.clear()
        _found[key] = find_best_time_window(coverage, window.visible)
    return _found[key]
